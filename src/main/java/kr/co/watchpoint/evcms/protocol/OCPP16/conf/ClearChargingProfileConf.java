...
{
	private Logger logger = LoggerFactory.getLogger("ClearChargingProfileConf");
	...
	public String process(AprotechWebSocketSession aprotechSession, EVCMSService evcmsService, String machineId, String dumy, String rawData)
	{
        ...
        try
        {
	        ...
	        ClearChargingProfileConf.Payload payload = gson.fromJson(payloadString, ClearChargingProfileConf.Payload.class);
	        
	        logger.info("================= ClearChargingProfileConf =================");
	        logger.info("machineId -> " + machineId);
	        logger.info("status -> " + payload.status);	        
	        logger.info("====================================================");
			
			AccessDAO dao = new AccessDAO(evcmsService);
			dao.addMachineClearChargingProfileLog(machineId, payload.status);
	        
        }catch(Exception e) {
        	logger.error(String.format("ClearChargingProfileConf.process() exception : %s", e.getMessage()));
        }		
