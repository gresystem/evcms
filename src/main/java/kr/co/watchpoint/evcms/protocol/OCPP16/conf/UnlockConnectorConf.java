...
public class UnlockConnectorConf 
{
	private Logger logger = LoggerFactory.getLogger("UnlockConnectorConf");
	...
	public String process(AprotechWebSocketSession aprotechSession, EVCMSService evcmsService, String machineId, String connectorId, String rawData)
	{
        ...
        try
        {
	        ...
	        UnlockConnectorConf.Payload payload = gson.fromJson(payloadString, UnlockConnectorConf.Payload.class);
	        
	        logger.info("================= UnlockConnectorConf =================");
			...
			dao.addMachineUnlockConnectorLog(machineId, connectorId, payload.status);
	        
        }catch(Exception e) {
        	logger.error(String.format("UnlockConnectorConf.process() exception : %s", e.getMessage()));
        }		
