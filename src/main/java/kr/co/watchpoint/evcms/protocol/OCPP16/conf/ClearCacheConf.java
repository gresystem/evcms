...
public class ClearCacheConf 
{
	private Logger logger = LoggerFactory.getLogger("ClearCacheConf");
	...
	public String process(AprotechWebSocketSession aprotechSession, EVCMSService evcmsService, String machineId, String dumy, String rawData)
	{
        ...
        try
        {
	        ...
	        ClearCacheConf.Payload payload = gson.fromJson(payloadString, ClearCacheConf.Payload.class);
	        
	        logger.info("================= ClearCacheConf =================");
	        logger.info("machineId -> " + machineId);
	        logger.info("status -> " + payload.status);	        
	        logger.info("====================================================");
			
			AccessDAO dao = new AccessDAO(evcmsService);
			dao.addMachineClearCacheLog(machineId, payload.status);
	        
        }catch(Exception e) {
        	logger.error(String.format("ClearCacheConf.process() exception : %s", e.getMessage()));
        }		
